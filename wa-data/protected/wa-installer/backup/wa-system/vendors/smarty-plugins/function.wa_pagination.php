...
function smarty_function_wa_pagination($params, &$smarty)
{
    ...
    $html = '<div class="back"><a href="'.'?page='.($page - 1).'">Предыдущая</a></div><div';
    ...
    if ($page > 1 && $prev) {
        ...
        //$html .= '<a class="inline-link" href="'.$page_url.'">'.$prev.'</a>';
    }
    $p = 1;
    $n = 1;
    while ($p <= $total) {
        if ($p > $nb && ($total - $p) > $nb && abs($page - $p) > $n && ($p < $page ? ($page - $n - $p > 1) : ($total - $nb > $p))) {
            ...
            $html .= '<a '.($p == $page ? ' class="active"' : '').' href="'.$page_url.'">'.$p.'</a>';
            $p++;
        }
    }
    ...
    if ($page < $total && $next) {
		...
		$html .= '<div class="forw"><a href="'.$page_url.'">Следующая</a></div>';
    }
